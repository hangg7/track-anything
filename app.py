...
def get_frames_from_video(video_input, video_state):
    ...
    if args.input_frame_pattern is not None:
        frame_paths = sorted(glob(args.input_frame_pattern))
        frames = [
...
def vos_tracking_video(video_state, interactive_state, mask_dropdown):
    ...
    if interactive_state["mask_save"]:
        if args.input_frame_pattern is None:
            ...
            assert output_mask_dir is not None
            ...
            if interactive_state["track_end_number"]:
                ...
                frame_paths = frame_paths[video_state["select_frame_number"] :]
                masks = masks[video_state["select_frame_number"] :]
            for frame_path, mask in zip(frame_paths, masks):
                cv2.imwrite(
                    os.path.join(
                        output_mask_dir,
                        os.path.basename(frame_path).split(".")[0] + ".png",
                    ),
            ...
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    template_frame = gr.Image(
                        ...
                        visible=False,
                    ).style(height=args.gui_img_height)
